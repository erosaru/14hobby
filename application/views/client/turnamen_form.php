...
<div class="span8">
	...
	<?if($this->uri->segment(2) == "admin_edit"){?>
		<? $url = base_url()."turnamen/admin_update/".$data[0]->id;?>
	<?}else{?>
		<? $url = base_url()."turnamen/admin_save/";}?>
	<form id="form_turnamen" class="form-horizontal" method="post" action="<?= $url;?>">
		...
		<div class="form-group">
			<label class="col-sm-3 control-label">Officer Turnamen</label>
			<div class="col-sm-9">
				<div class="col-sm-4" style="padding:0px;">
					<input type="text" value="<?php if(isset($data[0]->officer_turnamen)) echo $data[0]->officer_turnamen; else if(isset($data)) echo $data['officer_turnamen']?>" class="form-control" name="officer_turnamen" id="officer_turnamen">
				</div>
			</div>
		</div>
		<div class="form-group">
			<label class="col-sm-3 control-label">Keterangan Turnamen</label>
			<div class="col-sm-9">
		...
		<div class="form-group">
			...
			<div class="col-sm-9">
				<a class="btn btn-success" href="" onclick="before_save();return false;">Simpan</a> <a class="btn btn-danger" href="<?= base_url();?>turnamen/admin_index">Batal</a>
			</div>
		</div>	
	</form>
</div>
