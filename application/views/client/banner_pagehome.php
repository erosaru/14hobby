...
<h1>Banner Home</h1>
<form class="form" style="margin:0 auto; border:1px black solid; padding:10px;width:80%;" id="form_banner" class="form" method="post" action="<?= base_url();?>banner/save_pagehome" enctype="multipart/form-data" accept-charset="utf-8">
...
<script>
	...
	function save(){
		...
		$("#form_banner").attr('action', "<?= base_url()?>banner/save_pagehome");
		$("#form_banner").submit();
	}
	...
	function change_data(){		
		...
		switch(id){
		<? foreach($picture as $row){ ?>
			case "<?=$row->id?>": $("#img_alt").val("<?=$row->img_alt?>");tinyMCE.activeEditor.setContent("<?=mysql_real_escape_string($row->deskripsi)?>");$("#delete").html("<a href='<?=base_url()?>banner/delete_pagehome/<?=$row->id?>' class='btn btn-danger' onclick='return(confirm(\"Anda yakin akan menghapus banner ini?\"))'>Delete</a>");break;
		<?}?>
			default: $("#img_alt").val("");tinyMCE.activeEditor.setContent("");$("#delete").html("");break;
		}
	}
</script>