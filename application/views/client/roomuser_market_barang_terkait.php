...
<form action="<?=base_url()?>ensiklopedia-terkait" method="get" class="form">
...
<?foreach($item as $row):?>
	<div class="col-sm-4" style="border:1px solid black; margin-bottom:10px;min-height:200px;">
		...
		<div class="col-sm-12 text-center" style="margin-bottom:10px;">
			...
			<?if(empty($id)):?>
				<a href="<?=base_url()?>add-link-ensiklopedia/<?= $row->id?>" class="btn btn-success">Link</a>
			<?else:?>
				<a href="<?=base_url()?>add-link-ensiklopedia/<?= $row->id?><?if(!empty($id)) echo "?id=$id";?>" class="btn btn-success">Link</a>
			<?endif?>
