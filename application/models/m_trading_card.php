<?php
Class M_trading_card extends CI_Model {	
	...
	public function get_kategori($option = 0){
		if($option == 1)
			$this->db->where("ensiklopedia", 1);
		$query = $this->db->get("tbl_kategori_kartu");
		if($query->num_rows() > 0){
			foreach($query->result() as $row){
	...
	public function get_merk($option = 0){
		if($option == 1)
			$this->db->where("ensiklopedia", 1);
	...
	public function get_card_type(){
		$this->db->select("distinct(card_type)");
		$query = $this->db->get("tbl_kartu");
	...
	public function get_clan_type(){
		$this->db->select("distinct(clan_type)");
		$query = $this->db->get("tbl_kartu");
	...
	public function get_booster_set(){
		$this->db->select("distinct(booster_set)");
		$query = $this->db->get("tbl_kartu");
